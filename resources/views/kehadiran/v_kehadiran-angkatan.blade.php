        ...
        <nav>
            <div class="nav nav-tab nav-angkatan justify-content-center" id="nav-angkatan" role="tablist">
                @foreach($angkatan_aktif->sortBy('tahun_angkatan') as $data)
                    <a class="nav-item nav-link {{ request()->segment(3) == ($data->tahun_angkatan) ? 'active' : '' }}" id="nav-kehadiran{{$data->tahun_angkatan}}-tab" href="/kehadiran/angkatan/{{$data->tahun_angkatan}}/skema/s-0{{ $skemafirst_id }}/semester/{{ $data->semester_aktif }}" aria-controls="angkatan{{$data->tahun_angkatan}}" aria-selected="true">
                ...
                <div class="menu-semester">
                    <div class="list-group list-group-flush">
                        ...
                        @foreach($semester as $data)
                            <a class="list-group-item list-group-item-action {{ request()->segment(7) == ($data->id) ? 'active' : '' }}" id="nav-semester{{ $data->id }}-tab" href="/kehadiran/angkatan/{{ $t_a }}/skema/s-0{{ $skemafirst_id }}/semester/{{ $data->id }}" aria-controls="kehadiran-semeseter{{ $data->id }}" aria-selected="true">{{ $data->id }}</a>
                        @endforeach        
                    </div>
                </div>
