...
<head>
    ...
    <title>Kelulusan Rapor Peserta</title>
...
<body>
    <div class="head-content d-flex">
        <a style="font-size: 18px"><strong>Data Kelulusan Rapor Peserta Angkatan {{ $t_a }} Semester {{ $i_s }} {{ $spa ? '('.$spa.')' : ''}}</strong></a> 
    ...
    <table class="table table-bordered" cellspacing="0" id="peserta">
        <thead>
            <tr>
                ...
                <th class="align-middle nama" style="width: 11%;" rowspan="2">Kehadiran</th>
                <th class="align-middle nama" style="width: 11%;" rowspan="2">Kelengkapan Tugas</th>
                <th class="align-middle nama" style="width: 17%;" rowspan="2">Status Kelulusan</th>
            </tr>
        </thead>
        <tbody>
            @foreach( $rapor_kelulusan as $data)
            <tr> 
                <td>{{ $loop->iteration }}</td>
                <td>{{ $data->peserta->npm_peserta }}</td>
                <td>{{ $data->peserta->nama_peserta }}</td>
                <td>{{ $data->peserta->skema->nama_skema }}</td>
                <td>{{ $data->kehadiran }}</td>
                <td>{{ $data->kelengkapan_tugas }}</td>
