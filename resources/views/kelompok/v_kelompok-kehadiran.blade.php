        ...
        <div class="border row">
            <div class="menu-semester">
                <div class="list-group list-group-flush">
                    ...
                    <a class="list-group-item list-group-item-action {{ request()->segment(6) == ($data->id) ? 'active' : '' }}" id="nav-semester{{ $data->id }}-tab" href="/kelompok-asistensi/asisten/{{$user->id}}/kehadiran/semester/{{ $data->id }}" aria-controls="tugas-semeseter{{ $data->id }}" aria-selected="true">{{ $data->id }}</a>
                    @endforeach        
                </div>
            </div>
